...
describe('AgentManager', () => {
  ...
  it('splits submitted input into text and delayed carriage return', async () => {
    ...
    const agent = await manager.create(basePayload)
  ...
  it('updates codex model from terminal output', async () => {
    const manager = new AgentManager()
    const created = await manager.create({
      ...basePayload,
      provider: 'codex',
  ...
  it('ignores stale exit callbacks from replaced PTYs during restart', async () => {
    const manager = new AgentManager()
    const created = await manager.create({ ...basePayload, name: 'race-check' })
  ...
  it('does not let force-kill timeout terminate a newly restarted PTY', async () => {
    const manager = new AgentManager()
    const created = await manager.create({ ...basePayload, name: 'timeout-check' })
  ...
  it('treats an explicit stop as idle even when the PTY exits non-zero', async () => {
    const manager = new AgentManager()
    const created = await manager.create({ ...basePayload, name: 'stop-check' })
  ...
  it('enforces hard cap for concurrently active agents', async () => {
    const manager = new AgentManager()
    ...
    ).rejects.toThrow(`Maximum concurrent agents (${MAX_CONCURRENT_AGENTS_HARD_LIMIT}) reached`)
  })

  it('does not auto-start an idle session when active cap is reached', async () => {
    const manager = new AgentManager()
  ...
  it('allows restart of an existing running agent even when at active cap', async () => {
    const manager = new AgentManager()
    const ids: string[] = []
    for (let i = 0; i < MAX_CONCURRENT_AGENTS_HARD_LIMIT; i++) {
      const created = await manager.create({
  ...
  it('does not transition idle agent to starting when restart is blocked by cap', async () => {
    const manager = new AgentManager()
    for (let i = 0; i < MAX_CONCURRENT_AGENTS_HARD_LIMIT; i++) {
      await manager.create({
        ...basePayload,
  ...
  it('never exceeds hard cap under repeated spawn attempts', async () => {
    const manager = new AgentManager()
    ...
    const removed = await manager.remove(createdIds[0])
    expect(removed).toBe(true)
    expect(manager.activeCount()).toBe(MAX_CONCURRENT_AGENTS_HARD_LIMIT - 1)

    const extra = await manager.create({
      ...basePayload,
      name: 'one-more'
    })
